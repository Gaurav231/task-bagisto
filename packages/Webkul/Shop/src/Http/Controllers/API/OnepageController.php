...
class OnepageController extends APIController
{
     ...
     * Store order
     */
    public function storeOrder(Request $request): JsonResource
    {
        $deliveryInstructions = $request->input('delivery_instructions');
        if (Cart::hasError()) {
            return new JsonResource([
                'redirect'     => true,
                'redirect_url' => route('shop.checkout.cart.index'),
        ...
        if($deliveryInstructions){
            $order = $this->orderRepository->create(array_merge(Cart::prepareDataForOrder(), ['delivery_instructions' => $deliveryInstructions]));
        }else{
